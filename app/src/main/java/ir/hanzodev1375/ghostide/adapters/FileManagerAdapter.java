...
public class FileManagerAdapter extends RecyclerView.Adapter<FileManagerAdapter.ViewHolder> {
     ...
     * (modified/added/untracked/etc). Items matching these paths (or folders that contain such
     * items) are highlighted with the "modified" color. Pass an empty set to clear the highlighting
     * (e.g. after commit/push, or when the current directory is outside a git repository).
     */
    ...
    public void setupSelectionTracker(RecyclerView recyclerView) {
        ...
        ItemKeyProvider<Long> keyProvider = new ItemKeyProvider<
                Long>(ItemKeyProvider.SCOPE_CACHED) {
            @Nullable
            @Override
        ...
        ItemDetailsLookup<Long> detailsLookup = new ItemDetailsLookup<Long>() {
            @Nullable
            @Override
    ...
    public void search(String query) {
        ...
        DiffUtil.DiffResult diffResult = DiffUtil.calculateDiff(new FileDiffCallback(items, filteredList));
        items = filteredList;
        diffResult.dispatchUpdatesTo(this);
        if (selectionTracker != null) selectionTracker.clearSelection();
    }
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_file_manager, parent, false);
        ...
        void bindItem(FileManagerModel item, int position) {
            ...
            switch (item.getState()) {
                case CREATOR:
                    ...
                    break;
                case RENAME:
                    color = MaterialColors.getColor(tvName, com.google.android.material.R.attr.colorSecondary);
                    break;
                case SERACH:
                ...
                default:
                    if (isGitChanged(item)) {
                        ...
                        color = MaterialColors.getColor(tvName, com.google.android.material.R.attr.colorOnSurface);
                    }
            }
