...
public class CLanguage implements Language {
  ...
  private final CIncrementalAnalyzeManager manager;

  private final CQuoteHandler quoteHandler = new CQuoteHandler();

  public CLanguage() {
    autoComplete = new IdentifierAutoComplete(KEYWORDS);
    manager = new CIncrementalAnalyzeManager();
  }
  ...
  private int getIndentAdvance(String content) {
    CTextTokenizer t = new CTextTokenizer(content);
    CTokens token;
    int advance = 0;
    while ((token = t.nextToken()) != CTokens.EOF) {
