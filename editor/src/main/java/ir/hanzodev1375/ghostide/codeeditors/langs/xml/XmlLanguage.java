...
public class XmlLanguage implements Language {
  ...
  private static final CodeSnippet TAG_SNIPPET = CodeSnippetParser.parse("<${1:tag}>$0</${1:tag}>");

  private static final CodeSnippet SELF_CLOSE_TAG_SNIPPET = CodeSnippetParser.parse("<${1:tag} />");

  private static final CodeSnippet ATTRIBUTE_SNIPPET =
      CodeSnippetParser.parse("${1:name}=\"${2:value}\"");

  private static final CodeSnippet CDATA_SNIPPET =
      CodeSnippetParser.parse("<![CDATA[\n    $0\n]]>");
  private CustomFormatter formats;

  private static final CodeSnippet COMMENT_SNIPPET = CodeSnippetParser.parse("<!-- $0 -->");
  ...
  private final XmlIncrementalAnalyzeManager manager;

  private final XmlQuoteHandler quoteHandler = new XmlQuoteHandler();

  public XmlLanguage() {
    autoComplete = new IdentifierAutoComplete(KEYWORDS);
    manager = new XmlIncrementalAnalyzeManager();
    formats = new CustomFormatter();
    var xmlformat = new XmlFormatter();
    formats.setFormatAction(xmlformat::formatxml);
  }
  ...
  public void requireAutoComplete(
    ...
    if ("tag".startsWith(prefix) && !prefix.isEmpty())
      publisher.addItem(
          new SimpleSnippetCompletionItem(
              "tag",
              "Snippet - XML Tag",
              new SnippetDescription(prefix.length(), TAG_SNIPPET, true)));
    if ("self".startsWith(prefix) && !prefix.isEmpty())
      publisher.addItem(
          new SimpleSnippetCompletionItem(
              "self",
              "Snippet - Self Close Tag",
              new SnippetDescription(prefix.length(), SELF_CLOSE_TAG_SNIPPET, true)));
    if ("attr".startsWith(prefix) && !prefix.isEmpty())
      publisher.addItem(
          new SimpleSnippetCompletionItem(
              "attr",
              "Snippet - Attribute",
              new SnippetDescription(prefix.length(), ATTRIBUTE_SNIPPET, true)));
    if ("cdata".startsWith(prefix) && !prefix.isEmpty())
      publisher.addItem(
          new SimpleSnippetCompletionItem(
              "cdata",
              "Snippet - CDATA",
              new SnippetDescription(prefix.length(), CDATA_SNIPPET, true)));
  ...
  private int getIndentAdvance(String content) {
    int advance = 0;
    XmlTextTokenizer t = new XmlTextTokenizer(content);
    XmlTokens token;
    while ((token = t.nextToken()) != XmlTokens.EOF) {
      if (token == XmlTokens.TAG_OPEN) advance++;
      if (token == XmlTokens.TAG_CLOSE || token == XmlTokens.TAG_SELF_CLOSE) advance--;
    }
    return Math.max(0, advance) * 4;
  }

  private final NewlineHandler[] newlineHandlers = new NewlineHandler[] {new TagHandler()};
  ...
  class TagHandler implements NewlineHandler {
